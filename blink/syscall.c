...
#ifdef __wasm__
  /* B7: Pipe-blocking yield - if reading from guest-pipe and got EAGAIN,
   * transition to BLOCKED and yield to next runnable process. */
  if (rc == -EAGAIN && fd->guest_data && g_process_table.initialized && g_process_table.count > 1) {
    ...
    if (self && self->state == GUEST_PROC_RUNNABLE) {
      struct GuestProcess *nxt = self->next_runnable;
      ...
      if (nxt && nxt != self) {
        ERRF("B7-PIPE-BLOCK pid=%d fd=%d yielding to pid=%d", self->pid, fildes, nxt->pid);
        guest_proc_transition(self, GUEST_PROC_BLOCKED);
        self->block_reason = BLOCK_PIPE;
        self->block_fd = fildes;
        m->ip -= m->oplen;
        m->interrupted = true;
        g_process_table.current = nxt;
...
#ifdef __wasm__
  /* B7: Pipe wakeup - after successful write to guest-pipe, wake any processes
   * blocked reading from the same pipe (matching by guest_data pointer). */
  if (rc > 0 && fd->guest_data && g_process_table.initialized && g_process_table.count > 1) {
    int i;
    for (i = 0; i < MAX_GUEST_PROCESSES; i++) {
      struct GuestProcess *p = &g_process_table.procs[i];
      if (p->state == GUEST_PROC_BLOCKED &&
          p->block_reason == BLOCK_PIPE &&
          p->machine && p->machine->system) {
        struct Fd *blocked_fd;
        LOCK(&p->machine->system->fds.lock);
        blocked_fd = GetFd(&p->machine->system->fds, p->block_fd);
        if (blocked_fd && blocked_fd->guest_data == fd->guest_data) {
          ERRF("B7-PIPE-WAKE pid=%d fd=%d woke by writer pid=%d",
               p->pid, p->block_fd, g_process_table.current ? g_process_table.current->pid : 0);
          guest_proc_transition(p, GUEST_PROC_RUNNABLE);
          p->block_reason = BLOCK_NONE;
          p->block_fd = -1;
        }
        UNLOCK(&p->machine->system->fds.lock);
      }
    }
  }
#endif
...
static int SysFstat(struct Machine *m, i32 fd, i64 staddr) {
  ...
  if ((rc = VfsFstat(fd, &st)) != -1) {
    ...
    /* B7: Fallback for guest-side pipes not in VFS layer */
    struct Fd *f;
    LOCK(&m->system->fds.lock);
    f = GetFd(&m->system->fds, fd);
    if (f && f->guest_data) {
